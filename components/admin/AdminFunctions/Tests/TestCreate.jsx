  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'edit' && editingTest) {
        const response = await api.examPut(`test/${editingTest._id}`, testData);
        const data = response?.data;
        alert(`Тест успешно обновлен! ID: ${data?.testId || editingTest._id}`);
        if (onTestUpdated) {
          onTestUpdated(data);
        }
      } else {
        const response = await api.examPost('create-test', testData);
        const data = response?.data;
